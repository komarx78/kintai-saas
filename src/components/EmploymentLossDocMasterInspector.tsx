      ...
      <div className="bg-slate-900 p-3 rounded-2xl border border-slate-800 flex flex-wrap items-center justify-between gap-3 shadow-lg">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[11px] text-amber-400 font-bold bg-amber-950/60 px-2.5 py-1 rounded-lg border border-amber-800/80 flex items-center gap-1.5">
            ...
            様式第4号（第7条関係・帳票種別 17191）公式原本較正済
          </span>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className="grid grid-cols-3 gap-1.5 text-xs font-bold">
                <button
                  ...
                  className={`p-2 rounded-xl text-center transition cursor-pointer ${
                    selectedSection === 'header' ? 'bg-amber-600 text-white shadow-xs' : 'bg-slate-50 text-slate-700 hover:bg-slate-100'
                  }`}
                >
                  ① ヘッダー・番号
                </button>
                <button
                  ...
                  className={`p-2 rounded-xl text-center transition cursor-pointer ${
                    selectedSection === 'employee_basic' ? 'bg-amber-600 text-white shadow-xs' : 'bg-slate-50 text-slate-700 hover:bg-slate-100'
                  }`}
                >
                  ② 取得・離職日
                </button>
                <button
                  onClick={() => setSelectedSection('loss_detail')}
                  className={`p-2 rounded-xl text-center transition cursor-pointer ${
                    selectedSection === 'loss_detail' ? 'bg-amber-600 text-white shadow-xs' : 'bg-slate-50 text-slate-700 hover:bg-slate-100'
                  }`}
                >
                  ③ 喪失原因
                </button>
                <button
                  onClick={() => setSelectedSection('lower_table')}
                  className={`p-2 rounded-xl text-center transition cursor-pointer ${
                    selectedSection === 'lower_table' ? 'bg-amber-600 text-white shadow-xs' : 'bg-slate-50 text-slate-700 hover:bg-slate-100'
                  }`}
                >
                  ④ 下段確認欄
                </button>
                <button
                  ...
                  className={`p-2 rounded-xl text-center transition cursor-pointer ${
                    selectedSection === 'office' ? 'bg-amber-600 text-white shadow-xs' : 'bg-slate-50 text-slate-700 hover:bg-slate-100'
                  }`}
                >
                  ⑤ 事業主欄
                </button>
              ...
              <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs space-y-3 text-xs">
                ...
                {selectedField.pitch !== undefined && (
                  <div className="bg-amber-50/70 p-3 rounded-2xl border border-amber-200 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-mono text-xs font-black text-amber-700 bg-white px-2 py-0.5 rounded-md border border-amber-300">
                    ...
                    <input
                      ...
                      onChange={(e) => updateField(selectedField.id, 'pitch', parseFloat(e.target.value) || 2.86)}
                    ...
                    <div className="flex items-center justify-between gap-1 pt-0.5">
                      <button
                        ...
                        className="px-2 py-1 bg-white hover:bg-amber-100 active:scale-95 text-amber-800 border border-amber-300 rounded-lg text-[10px] font-black shadow-2xs cursor-pointer"
                        title="0.05% 狭くする"
                      >
                        -0.05
                      </button>
                      ...
                      <input
                        ...
                        value={selectedField.pitch}
                        onChange={(e) => updateField(selectedField.id, 'pitch', parseFloat(e.target.value) || 2.86)}
                        className="w-16 bg-white border border-amber-300 rounded-lg p-1 text-center text-xs font-mono font-bold text-amber-900"
                      />
                      <button
                        type="button"
                ...
                <div className="pt-2 border-t border-slate-100 flex items-center justify-between gap-2">
                  ...
                  <button
                    onClick={handleSave}
                    disabled={isSaving}
                    className="flex-1 px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-xl font-black text-xs transition cursor-pointer flex items-center justify-center gap-1.5 shadow-sm"
                  >
          ...
          <div className="lg:col-span-8 flex flex-col items-center">
            ...
            <div className="mb-2 w-full flex flex-wrap items-center justify-between gap-2 px-2">
              ...
              <div className="flex items-center gap-2 bg-amber-50 text-amber-900 border border-amber-200 px-3 py-1.5 rounded-xl text-[11px] font-bold shadow-2xs">
