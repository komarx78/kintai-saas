...
const ShiftEmployeeMaster: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data: usersData } = await supabase.from('users').select('id, name, role, has_shift_access').eq('tenant_id', tenantId);
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                <tr><td colSpan={7} className="p-8 text-center"><div className="animate-spin w-8 h-8 border-4 border-indigo-500 border-t-transparent rounded-full mx-auto"></div></td></tr>
              ) : employees.length === 0 ? (
                <tr><td colSpan={7} className="p-8 text-center text-slate-500 font-bold">システムに従業員が登録されていません。「従業員を新規招待」から登録してください。</td></tr>
              ) : (
                employees.map(emp => (
